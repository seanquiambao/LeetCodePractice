...
typedef unordered_map<char,char> hashTable;
class Solution {
public:
    void mapCharacters(hashTable& hash, string key) {
        char letter = 'a';
        char end = 'z' + 1;
        int keySize = key.size();
        for(int i = 0; (i < keySize) && (letter != end); ++i) {
            if(auto search = hash.find(key[i]); search == hash.end() && isalpha(key[i])) { 
                hash.insert({key[i], letter++});
            }
        }
    }
    string decodeMessage(string key, string message) {
        hashTable hash;
        mapCharacters(hash, key);

        int messageSize = message.size();
        string ans = message;
        for(int i = 0; i < messageSize; ++i) {
            if(isalpha(ans[i])) { 
                ans[i] = hash.find(ans[i])->second;
            }
        }
