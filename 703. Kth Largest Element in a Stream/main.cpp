...
private:
    int currentK;
public:
    int getKValue() { return currentK; }
    void setKValue(int k) { currentK = k; }
    ...
    KthLargest(int k, vector<int>& nums) {
        ...
        int kthIndex = lastIndex - k + 1;
        kthIndex = kthIndex < 0 ? 0 : kthIndex;

        for(int i = lastIndex; i >= kthIndex ; --i) {
            pq.push(nums[i]);
        }
    }
...
int main() {
    ...
    cout << kthLargest->add(-1) << endl;   // return 4
    cout << kthLargest->add(1) << endl;   // return 5
    cout << kthLargest->add(-2) << endl;  // return 5
    cout << kthLargest->add(-4) << endl;   // return 8
    cout << kthLargest->add(3) << endl;   // return 8
